...
describe('nudgeAgent', () => {
  it('sends POST with message body', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/agents/agent-123/nudge', expect.objectContaining({
      method: 'POST',
...
describe('killAgent', () => {
  it('sends POST to correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/agents/agent-456/kill', expect.objectContaining({
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    }));
  });
...
describe('decomposeGoal', () => {
  it('sends POST to correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/goals/goal-1/decompose', expect.objectContaining({
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    }));
...
describe('dispatchGoal', () => {
  it('sends POST to correct endpoint', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/goals/goal-2/dispatch', expect.objectContaining({
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    }));
    expect(result).toEqual(agents);
